...
ObjectWireTile::ObjectWireTile(const irr::core::vector2df& apos, const irr::core::vector2di& rpos, bool _near)
      ...
      _near(_near)
{
    ...
    //const unsigned int objectWireNum = _near ? Settings::getInstance()->objectWireNearNum : Settings::getInstance()->objectWireNum;
    const unsigned int objectWireSize = _near ? Settings::getInstance()->objectWireNearSize :Settings::getInstance()->objectWireSize;
    ...
    const ObjectPoolManager::objectPoolList_t& objectPoolList = _near ? ObjectPoolManager::getInstance()->getObjectPoolNearList() : ObjectPoolManager::getInstance()->getObjectPoolFarList();
        
    for (ObjectPoolManager::objectPoolList_t::const_iterator opIt = objectPoolList.begin();
         opIt != objectPoolList.end();
         opIt++)
    {
        ObjectPool* objectPool = *opIt;
...
{
    const float objectWireSize = _near ? (float)Settings::getInstance()->objectWireNearSize : (float)Settings::getInstance()->objectWireSize;
...
ObjectWireNear* ObjectWireNear::objectWireNear = 0;

void ObjectWireNear::initialize()
{
    if (objectWireNear == 0)
    {
        objectWireNear = new ObjectWireNear();
    }
}

void ObjectWireNear::finalize()
{
    if (objectWireNear)
    {
        delete objectWireNear;
        objectWireNear = 0;
    }
}
...
ObjectWireNear::ObjectWireNear()
    : tiles(0),
      lastWireCenter()
{
    const unsigned int objectWireNearNum = Settings::getInstance()->objectWireNearNum;
    assert(objectWireNearNum);
    
    tiles = new ObjectWireTile*[objectWireNearNum*objectWireNearNum];
    memset(tiles, 0, objectWireNearNum*objectWireNearNum*sizeof(ObjectWireTile*));
}
...
ObjectWireNear::~ObjectWireNear()
{
    ...
    if (tiles)
    {
        for (unsigned int x = 0; x < objectWireNearNum; x++)
        {
            for (unsigned int y = 0; y < objectWireNearNum; y++)
            {
                if (tiles[x + objectWireNearNum*y])
                {
                    delete tiles[x + objectWireNearNum*y];
                    tiles[x + objectWireNearNum*y] = 0;
                }
            }
        }
        delete tiles;
...
void ObjectWireNear::reset()
{
    const unsigned int objectWireNearNum = Settings::getInstance()->objectWireNearNum;
    dprintf(MY_DEBUG_NOTE, "ObjectWireNear::reset()\n");
...
bool ObjectWireNear::update(const irr::core::vector3df& newPos, bool force)
{
    const unsigned int objectWireNearNum = Settings::getInstance()->objectWireNearNum;
    const unsigned int objectWireNearSize = Settings::getInstance()->objectWireNearSize;
    assert(objectWireNearNum);

    bool ret = false;

    irr::core::vector2di newWireCenter = irr::core::vector2di((int)newPos.X/(int)objectWireNearSize, (int)newPos.Z/(int)objectWireNearSize);
    
    if (newWireCenter != lastWireCenter)
    {
        dprintf(MY_DEBUG_NOTE, "ObjectWireNear::update(): (%d, %d) -> (%d, %d)\n",
            lastWireCenter.X, lastWireCenter.Y, newWireCenter.X, newWireCenter.Y);
        irr::core::vector2di offset = newWireCenter - lastWireCenter;
        ObjectWireTile** newTiles = new ObjectWireTile*[objectWireNearNum*objectWireNearNum];
        memset(newTiles, 0, objectWireNearNum*objectWireNearNum*sizeof(ObjectWireTile*));
        
        // search for overlap in old tiles, if no overlap delete
        for (unsigned int y = 0, fy = 0; y < objectWireNearNum; y++, fy+=objectWireNearNum)
        {
            for (unsigned int x = 0; x < objectWireNearNum; x++)
            {
                unsigned int newX = x - offset.X;
                unsigned int newY = y + offset.Y;
                unsigned int oldTileNum = x + fy;
                if (newX < objectWireNearNum && newY < objectWireNearNum && tiles[oldTileNum/*x + objectWireNum*y*/])
                {
                    // overlap
                    newTiles[newX + objectWireNearNum*newY] = tiles[oldTileNum/*x + objectWireNum*y*/];
                    newTiles[newX + objectWireNearNum*newY]->rpos = irr::core::vector2di(newX, newY);
                }
                else
                {
                    if (tiles[oldTileNum/*x + objectWireNum*y*/])
        ...
        for (unsigned int y = 0, fy = 0; y < objectWireNearNum; y++, fy += objectWireNearNum)
        {
            for (unsigned int x = 0; x < objectWireNearNum; x++)
            {
                ...
                if (tiles[tileNum/*x + objectWireNum*y*/] == 0)
                {
                    tiles[tileNum/*x + objectWireNum*y*/] = new ObjectWireTile(
                        irr::core::vector2df(
                            (float)((lastWireCenter.X-(int)(objectWireNearNum/2)+(int)x)*(int)objectWireNearSize),
                            (float)((lastWireCenter.Y-1+(int)(objectWireNearNum/2)-(int)y)*(int)objectWireNearSize)),
                        irr::core::vector2di(x, y), true);
                }
            }
        }
        ret = true;
    ...
    const float newDistance = (float)(objectWireNearNum*objectWireNearSize) * 0.75f;
    const float rate = newDistance / TheGame::getInstance()->getCamera()->getFarValue();
    const irr::core::vector3df& cpos = TheGame::getInstance()->getCamera()->getPosition();
    irr::core::aabbox3d<irr::f32> cbbox = TheGame::getInstance()->getCamera()->getViewFrustum()->getBoundingBox();
